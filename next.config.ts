...
//   - Next.js 自身が __next_f のブートストラップをインラインで出す
...
const nextConfig: NextConfig = {
  ...
  experimental: {
    ...
    /*
     * 一度描いたページを、クライアント側のルーターキャッシュで 15 秒だけ再利用する。
     *
     * このアプリは全ルートが動的(ルートレイアウトが auth() を呼ぶ)なので、既定(dynamic: 0)では
     * 下部ナビでの行き来のたびにサーバへ取りに行き、そのあいだ loading.tsx の骨格が出る。
     * 骨格を一度出すと React のフォールバック抑制(FALLBACK_THROTTLE_MS = 300ms)が乗るため、
     * サーバがどれだけ速くても本文表示は約 330ms で頭打ちになる(2026-08-19 実測)。
     * キャッシュから描けばサーバへ行かず骨格も出ないので、この 330ms がまるごと消える。
     *
     * 15 秒なのは本番ログの実測から。同じ一覧へ戻ってくるまでの間隔は 5 秒以内が 18.2%、
     * 15 秒以内が 23.1%、30 秒以内が 26.0%(2026-09-07、実遷移 242 組)。効果の大半は
     * 短い間隔=タブの行き来に集まっていて、15 秒あれば 30 秒ぶんの効果のほぼ全てを拾える。
     * 長くするほど拾える組はわずかしか増えず、古い値を見せる時間だけが伸びる。
     *
     * 鮮度: 一覧(記録・デッキ)はサーバで取った1ページ目を最初の描画にだけ使い、マウント直後に
     * 裏で取り直す(Records / Decks の isRefreshing)ので、再利用されても表示は最新に追いつく。
     * ダッシュボードの各パネルはサーバ値をそのまま出すため、最大 15 秒ぶん古い値が出うる
     * (いずれも対戦を記録したときしか動かない値なので、この幅は許容する)。
     */
    staleTimes: {
      dynamic: 15,
      static: 180,
