...
//
// 週の境界は常に JST で判定する。端末のタイムゾーンで判定すると、海外在住のユーザーで
// JST 月曜の 0:00〜9:00 に「今週」が1週前へずれる（utils/yearMonth.ts と同じ規約）。

import { toJSTDateString, todayJSTDateString } from "@app/utils/date";

// 暦日 "YYYY-MM-DD" に days 日を足す(負数で過去へ)。
// 暦日をUTCとして扱うため、端末のタイムゾーンにも夏時間にも影響されない。
...
export function generateWeekOptions(count = 12): { value: string; label: string }[] {
  ...
  const monday = currentWeekValue();

  for (let i = 0; i < count; i++) {
    const value = addDays(monday, -i * 7);
    options.push({ value, label: `${weekRangeLabel(value)} の週` });
  }
