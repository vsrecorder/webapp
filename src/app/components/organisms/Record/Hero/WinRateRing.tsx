...
/*
 * 勝率を持たないときのリング色。算出できない状態なので勝ち/負けの色は使わず、
...
type Props = {
  ...
  teamWinRate?: number;
  // 勝率を持たないか(対戦が1件も無い / 引き分けだけで決着した対戦が無い)。
  // グレーのトラックだけのリングにし、中央の数字も「-」にする
  // (0% と書くと全敗と読めてしまう)
  empty?: boolean;
};
 ...
 *
 * 勝率を持たない記録(empty。対戦0件、または引き分けだけで決着した対戦が無い)では、
 * 円弧を描かずグレーのトラックだけにして中央を「-」にする。
 * 勝率0%として描くと全敗と読めてしまうため。
 *
