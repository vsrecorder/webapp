 ...
 *
 * 本番ビルド・CPU 4x・並の 4G での実測(2026-09-08、3回の中央値)では、ホームの中身が
 * 出揃うまでが 6.65 → 5.73 秒、ページ全体の DOM ノードが 2107 → 1807(-14%)、
 * RSC が 222 → 209KB。
 *
 * 伸ばすと登録店舗ぶんの件数がそのまま増えてパネルが読めなくなるため、既定はここで止める。
 * 2026-09-18 に一覧を内部スクロールから日付ごとのアコーディオンへ変えた(理由は MyGymPanel の
 * コメント)ので、**日数はそのままパネルの高さになる**。当時の「内部スクロールだから
 * 件数を半分にしても高さは変わらない」という理屈はもう成り立たない。14日へ戻すと
 * 畳んだ日付行が最大7本増えて、パネルが 300px ほど伸びる。
 */
