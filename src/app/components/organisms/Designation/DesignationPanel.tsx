...
} from "@app/components/organisms/Designation/Skeleton/DesignationPanelSkeleton";
import { usePlayerLinkedHint } from "@app/utils/playerLinkCache";
import { useCityleagueResultsHeight } from "@app/utils/cityleagueResultsHeightCache";
...
// count個のティアを DESIGNATION_LADDER_ROW_SIZE 個ずつの行に分割する(tier昇順のまま)。
...
export default function DesignationPanel({
  ...
  // 入賞したシティリーグの節の高さ。PlayerCityleagueResults が前回描画できた高さを
              ...
              <Fragment key={rowIndex}>
                <div className="flex items-stretch gap-1">
                  {displayRow.map((item, i) => {
                    const originalIndex =
                      rowIndex * DESIGNATION_LADDER_ROW_SIZE + (reversed ? row.length - 1 - i : i);
                    return (
