 ...
 *
 * タップでデッキ詳細モーダルを開く導線も持っていたが、呼び出し元(記録詳細ページ・記録情報
 * モーダル)はどちらも無効を指定していて到達しなかったため 2026-09-23 に外した。この部品を
 * 包む UsedDeckById が使用デッキ編集モーダルの導線を持っており、デッキ詳細はヒーローの
 * 「使用デッキ」から開ける。
 */
  ...
  if (!deck) {
    return (
      /* 枠(Card)では包まない。デッキコード未登録のときに DeckCodeCard が出す案内と
         ...
         タップは親(UsedDeckById)が受けて使用デッキ編集モーダルを開く */
      <div className="group w-full flex flex-col items-center gap-2 rounded-xl border-2 border-dashed border-primary/30 bg-primary/5 px-4 py-6 transition-colors hover:border-primary/60 hover:bg-primary/10 active:opacity-80">
        ...
        <div className="font-bold text-tiny text-primary">使用したデッキを登録しよう</div>
